...
<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;background:#fff">
  ...
  <div style="background:#062A5E;padding:28px 32px;border-radius:12px 12px 0 0">
    ...
    <div style="color:#fff;font-size:22px;font-weight:900">${company}</div>
  ...
  <div style="padding:28px 32px;border:1px solid #e2e8f0;border-top:none;border-radius:0 0 12px 12px">
    ...
    <p style="font-size:15px;color:#64748b;margin:0 0 24px">Please find your estimate below. You can review and approve it at any time using the button at the bottom.</p>
    ...
    <div style="border:1px solid #e2e8f0;border-radius:10px;overflow:hidden;margin-bottom:24px">
      <table style="width:100%;border-collapse:collapse">
        <tr><td style="padding:11px 16px;font-size:14px;color:#64748b">Subtotal</td><td style="padding:11px 16px;font-size:14px;text-align:right">${fmt(subtotal)}</td></tr>
        <tr><td style="padding:11px 16px;font-size:14px;color:#64748b;border-top:1px solid #f1f5f9">TPS (${tpsRate}%)</td><td style="padding:11px 16px;font-size:14px;text-align:right;border-top:1px solid #f1f5f9">${fmt(tps)}</td></tr>
        <tr><td style="padding:11px 16px;font-size:14px;color:#64748b;border-top:1px solid #f1f5f9">TVQ (${tvqRate}%)</td><td style="padding:11px 16px;font-size:14px;text-align:right;border-top:1px solid #f1f5f9">${fmt(tvq)}</td></tr>
        <tr style="background:#062A5E">
          <td style="padding:13px 16px;font-size:15px;color:#fff;font-weight:900">Total</td>
          <td style="padding:13px 16px;font-size:15px;color:#fff;font-weight:900;text-align:right">${fmt(total)}</td>
        </tr>
      </table>
    </div>
    ...
    <div style="background:#eff6ff;border:1px solid #dbeafe;border-radius:10px;padding:14px 18px;margin-bottom:24px">
      <div style="font-size:12px;color:#3b82f6;font-weight:700;text-transform:uppercase;letter-spacing:.5px">Deposit to confirm</div>
      <div style="font-size:22px;font-weight:900;color:#1d4ed8;margin:4px 0">${fmt(deposit)}</div>
    ...
    <div style="text-align:center;margin:28px 0">
      <a href="${portalLink}" style="background:#062A5E;color:#fff;text-decoration:none;padding:16px 36px;border-radius:10px;font-size:16px;font-weight:800;display:inline-block;letter-spacing:-.2px">
        ${deposit > 0 ? `Review, Approve &amp; Pay Deposit →` : `Review &amp; Approve Your Estimate →`}
      </a>
      <div style="margin-top:10px;font-size:12px;color:#94a3b8">Secure client portal · No account required</div>
    </div>

    <div style="margin-top:20px;padding-top:16px;border-top:1px solid #e2e8f0;font-size:13px;color:#94a3b8;text-align:center">
      ${cfg.email ? cfg.email : ''}${cfg.email && cfg.phone ? ' &nbsp;·&nbsp; ' : ''}${cfg.phone ? cfg.phone : ''}
    </div>
  ...
  await sb.from('estimates').update({ status: 'Sent', updated_at: new Date().toISOString() })
    .eq('id', estimate_id).neq('status', 'Accepted')
