  ...
  if (client?.email) {
    await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        to: client.email,
        subject: `✅ Estimate Approved — ${est.title}`,
        html: `
<div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto;background:#fff">
  <div style="background:#062A5E;padding:28px 32px;border-radius:12px 12px 0 0;text-align:center">
    ...
    <h2 style="color:#fff;margin:0;font-size:22px">Estimate Approved!</h2>
    <p style="color:#93c5fd;margin:8px 0 0;font-size:14px">${company}</p>
  </div>
  <div style="padding:28px 32px;border:1px solid #e2e8f0;border-top:none;border-radius:0 0 12px 12px">
    <p style="font-size:16px;color:#334155;margin:0 0 8px">Hi <strong>${client.name}</strong>,</p>
    <p style="font-size:15px;color:#64748b;margin:0 0 24px">Your estimate has been confirmed. Here's your summary:</p>
    ...
    <div style="text-align:center;margin:24px 0">
      ...
      <div style="margin-top:8px;font-size:12px;color:#94a3b8">Track payments &amp; upcoming appointments</div>
    </div>
    <div style="font-size:13px;color:#94a3b8;text-align:center;margin-top:16px;padding-top:16px;border-top:1px solid #f1f5f9">
      Questions? ${cfg.email ? cfg.email : ''}${cfg.email && cfg.phone ? ' · ' : ''}${cfg.phone ? cfg.phone : ''}
    </div>
  </div>
</div>`,
      }),
...
<div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto;padding:24px;background:#fff">
  <div style="background:#062A5E;border-radius:12px;padding:20px 24px;margin-bottom:24px">
    <h2 style="color:#fff;margin:0;font-size:20px">✅ Estimate Approved</h2>
  </div>
  <p style="font-size:15px;color:#333">
    <strong>${client?.name || 'Your client'}</strong> approved <strong>${est.title}</strong>
    for <strong style="color:#10b981">${fmt(total)}</strong>.
  </p>
